	...
	<script type="text/javascript">
		$(document).ready(function() {
			$("#btnExport").click(function(){
				top.$.jBox.confirm("确认要导出微博数据处理列表吗？","系统提示",function(v,h,f){
					if(v=="ok"){
						$("#searchForm").attr("action","${ctx}/weibo/weiBoStatusDispose/export");
						$("#searchForm").submit();
					}
				},{buttonsFocus:1});
...
<ul class="nav nav-tabs">
	<li class="active"><a href="${ctx}/weibo/weiBoStatusDispose/">微博危机数据处理列表</a></li>
</ul>
<form:form id="searchForm" modelAttribute="weiBoStatusDispose" action="${ctx}/weibo/weiBoStatusDispose/" method="post"
	...
	<table class="ul-form">
		<td>编号:</td>
		<td><input name="autoId" type="text" style="width:40px;" value="${query.autoId}"/></td>
		<td><select name="crisisLevel" style="width:80px;text-align:center;">
			<option value="">级别</option>
			<option value="1" <c:if test="${ query.crisisLevel ==1}">selected = "selected"</c:if>> 一级</option>
			<option value="2" <c:if test="${ query.crisisLevel ==2}">selected = "selected"</c:if>> 二级</option>
			<option value="3" <c:if test="${ query.crisisLevel ==3}">selected = "selected"</c:if>> 三级</option>
			<option value="4" <c:if test="${ query.crisisLevel ==4}">selected = "selected"</c:if>> 四级</option>
			<option value="5" <c:if test="${ query.crisisLevel ==5}">selected = "selected"</c:if>> 五级</option>
			<option value="0" <c:if test="${ query.crisisLevel ==0}">selected = "selected"</c:if>> 非危机</option>
		</select>
		</td>
		<td>
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
	...
	<tr>
		...
		<th>客服</th>
		<th>原文</th>
		<th class="sort-column s.created_at">发文时间</th>
		<%-- <th>动作状态</th>--%>
		<th>问题状态</th>
		<th>客户状态</th>
		<th>回复内容</th>
	...
	<c:forEach items="${page.list}" var="i" varStatus="status">
		<tr>
			<td>${i.autoId}</td>
			<td>
					${fns:getDictLabel(i.crisisLevel,'crisis_level','')}
			</td>
			<td>
					${i.user.name}
			</td>
			...
			<td>
				<fmt:formatDate value="${i.weiBoStatus.createdAt}" pattern="yyyy-MM-dd HH:mm:ss"/>
			</td>
			<td>
			...
			<shiro:hasPermission name="weibo:weiBoStatusDispose:edit">
				<td>
					<a  class='btn btn-primary active' type="submit" style='width:40px' href="${ctx}/weibo/weiBoStatusDispose/replyForm?id=${i.id}">处理</a>
				</td>
			</shiro:hasPermission>
			<td>
				...
				<form:form id="${i.id}" modelAttribute="weiBoStatusDispose" action="${ctx}/weibo/weiBoStatusDispose/update" class="form-horizontal">
