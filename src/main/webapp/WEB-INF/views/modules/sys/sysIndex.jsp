	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			function getNotifyNum(){
				$.get("${ctx}/oa/oaNotify/self/crisis/count?updateSession=0&t="+new Date().getTime(),function(data){
			...
			// 获取危机数据通知数目
			function getNotifyCrisisDataNum(){
				$.get("${ctx}/oa/oaNotify/self/crisis/count?updateSession=0&t="+new Date().getTime(),function(data){
					var num = parseFloat(data);
					if (num > 0){
						$("#notifyNum,#notifyNum1").show().html("("+num+")");
					}else{
						$("#notifyNum,#notifyNum1").hide()
					}
				});
			}
			getNotifyCrisisDataNum();
			setInterval(getNotifyCrisisDataNum, ${oaNotifyRemindInterval});

       		// 获取回复通知数目
			function getNotifyReplyNum(){
				$.get("${ctx}/oa/oaNotify/self/reply/count?updateSession=0&t="+new Date().getTime(),function(data){
					var num = parseFloat(data);
					if (num > 0){
			...
			getNotifyReplyNum();
			setInterval(getNotifyReplyNum, ${oaNotifyRemindInterval});

			// 获取@通知数目
			function getNotifyAtNum(){
				$.get("${ctx}/oa/oaNotify/self/at/count?updateSession=0&t="+new Date().getTime(),function(data){
					var num = parseFloat(data);
					if (num > 0){
			...
			getNotifyAtNum();
			setInterval(getNotifyAtNum, ${oaNotifyRemindInterval});

			// 获取私信通知数目
			function getNotifyMailNum(){
				$.get("${ctx}/oa/oaNotify/self/mail/count?updateSession=0&t="+new Date().getTime(),function(data){
					var num = parseFloat(data);
					if (num > 0){
						$("#notifyNum,#notifyNum4").show().html("("+num+")");
					}else{
						$("#notifyNum,#notifyNum4").hide()
					}
				});
			}
			getNotifyMailNum();
			setInterval(getNotifyMailNum, ${oaNotifyRemindInterval});
		});
	...
	<div id="main">
				...
				<ul id="userControl" class="nav pull-right">
					...
					<li id="userInfo" class="dropdown">
						...
						<ul class="dropdown-menu">
							...
							<li><a href="${ctx}/oa/oaNotify/self?type=0" target="mainFrame"><i class="icon-bell"></i>&nbsp; 危机数据 <span id="notifyNum1" class="label label-info hide"></span></a></li>
							<li><a href="${ctx}/oa/oaNotify/self?type=1" target="mainFrame"><i class="icon-bell"></i>&nbsp; 评论 <span id="notifyNum2" class="label label-info hide"></span></a></li>
							<li><a href="${ctx}/oa/oaNotify/self?type=2" target="mainFrame"><i class="icon-bell"></i>&nbsp; @我的 <span id="notifyNum3" class="label label-info hide"></span></a></li>
							<li><a href="${ctx}/oa/oaNotify/self?type=3" target="mainFrame"><i class="icon-bell"></i>&nbsp; 私信 <span id="notifyNum4" class="label label-info hide"></span></a></li>
						</ul>
					</li>
	    ...
	    <div class="container-fluid">
		    ...
		    <div id="footer" class="row-fluid">
	            Copyright &copy; 2012-${fns:getConfig('copyrightYear')} ${fns:getConfig('productName')} - Powered By <a href="http://www.inter3i.com/" target="_blank">北京博晓通科技有限公司</a> ${fns:getConfig('version')}
			</div>
		</div>
	</div>
	<script type="text/javascript"> 
